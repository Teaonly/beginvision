...
class DT_Sift {
public:
    DT_Sift(int numOctaves = 4, int numLevels = 5, int minLevel = 0, int minOctave = 0, int S = 3): 
            numOctaves_(numOctaves), numLevels_(numLevels), minLevel_(minLevel), minOctave_(minOctave), S_(S) { 
...
public:
    int run(Eigen::MatrixXd& img) {
        ...
        // 1. upsamle/downsample the original image, TODO
...
private:
    ...
    */
    void buildOctaves( Eigen::MatrixXd& I) {
        ...
        double bottomSigma = sigma0_ * powf(2, minOctave_ + 1.0 * minLevel_ / S_);
        ...
        for ( int oi = 0; oi < numOctaves_; oi++) {
            ...
            for ( int li = 1; li < numLevels_; li++) {
                double currentSigma = powf(2, currentOctave + (minLevel_ + li) / S_);
